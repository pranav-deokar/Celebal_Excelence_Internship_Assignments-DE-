...
-- All required SQL analysis queries from Part 3.

-- 01_total_revenue_per_category
...
-- 02_top_10_customers
SELECT o.customer_id, ROUND(SUM((oi.quantity * oi.unit_price * (1 - oi.discount_percent / 100.0))),2) AS total_order_value
...
joined AS (
    SELECT m.year, m.month, m.revenue,
           p.revenue AS prev_year_revenue
    FROM monthly m
...
SELECT co.cohort_month, cs.cohort_customers,
       SUM(CASE WHEN co.month_number=0 THEN 1 ELSE 0 END) AS month_0,
       SUM(CASE WHEN co.month_number=1 THEN 1 ELSE 0 END) AS month_1,
       SUM(CASE WHEN co.month_number=2 THEN 1 ELSE 0 END) AS month_2,
       SUM(CASE WHEN co.month_number=3 THEN 1 ELSE 0 END) AS month_3,
       ROUND(100.0*SUM(CASE WHEN co.month_number=0 THEN 1 ELSE 0 END)/cs.cohort_customers,2) AS retention_0_pct,
       ROUND(100.0*SUM(CASE WHEN co.month_number=1 THEN 1 ELSE 0 END)/cs.cohort_customers,2) AS retention_1_pct,
       ROUND(100.0*SUM(CASE WHEN co.month_number=2 THEN 1 ELSE 0 END)/cs.cohort_customers,2) AS retention_2_pct,
       ROUND(100.0*SUM(CASE WHEN co.month_number=3 THEN 1 ELSE 0 END)/cs.cohort_customers,2) AS retention_3_pct
